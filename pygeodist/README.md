...
`EARTH_RADIUS_METERS` constant from the Rust core plus a Rust-backed `Point`
wrapper and `geodesic_distance` while additional kernels are built out.
...
- Public exports today: `EARTH_RADIUS_METERS`, `GeodistError`, `InvalidGeometryError`, the `Point` wrapper, and `geodesic_distance(point_a, point_b)` returning meters with WGS84 mean radius.
- Non-goals: mirroring Shapely's API, accepting free-form geometry tuples, or silently coercing unsupported shapes.
- Future Python surface (gated on Rust kernels, no promised timeline): wrappers for bearings alongside `geodesic_distance`, Hausdorff (including bounding-box-clipped variants), and additional geometry wrappers once Rust exposes them.
